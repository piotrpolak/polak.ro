...
In past days HTTP APIs used to be either some random collections of RPC like endpoints of some heavy SOAP interfaces.
...
The business word still often preferred SOAP as had a great advantage over REST - SOAP web services were formally defined by
**machine-readable description language** - WSDL - which allowed the code generation tools to take care of all the glue code.

SOAP was not widely adopted by web and mobile applications due to its complexity and high verbosity.
...
Around 2011 Tony Tam created an open-sourced Swagger API. In the early days, Swagger was mostly used together with a
...
**TIP**: A well written Swagger/OpenAPI definition can be imported as a Postman collection and used out of box.
{: .notice--info}
...
Projects that adopted Swagger UI usually derived the API definition by scanning the existing source cod. Or in the
worst case maintaining source code and the Swagger definition independently.
...
Despite the multiple advantages, the approach where the API definition is just a second class citizen has some non
...
- Generate server skeleton + generate client SDKs + generate stubs/mocks for your clients to start the integration.

  For example: at this stage, in a typical web application frontend application development can be started parallel to
...
- When using a generated SDK, from the client point of view, remote services can be handled as if they were some local repositories
- Improved developer confidence and satisfaction

## Code generation

Having machine-readable API definition made it possible to avoid human-developed API glue code and it's **costly maintenance**
(sometimes done independent for each component). 
...
Code generation is actually the first place where **API First approach gains the full power** - it indirectly
...
- Explicit `ResponseType` — if you like void methods together `@ResponseStatus` annotation — you will have to get used
...
- [JSR Bean validation](https://beanvalidation.org/1.0/spec/) rules are leaked into the contract details
- Code formatting issues and unused imports
- Code spoiled with `@Api*` annotations belonging to Swagger 2.0 — these come from an
  outdated [swagger-core](https://mvnrepository.com/artifact/io.swagger/swagger-core) dependency from 2017. The
  annotations are only relevant when you use SpringFox (out-of-box SwaggerUI integration with application bootstrap
  classpath scanning TODO CHECK DEFINITION) but this is definitively useless when you already have a well-crafted
  definition available at your hand.

### Generating client SDK for backend-to-backend communication - Java

Client SDKs can be generated around multiple. HTTP libraries in mind such as:
- `jersey1`
- `jersey2`
- `feign`
- `okhttp-gson` (default),
...
From the client perspective, **in most cases it doesn’t matter which underlying HTTP library is used as long as the runtime exceptions are not handled manually from code**.
The general rule to pick the library is to avoid additional library dependencies — use what you already have in the project.

Please note that some libraries do not allow to be customized externally — this might be problematic if you expect to add additional
interceptors.

#### Adding custom headers

There might be a need to add custom headers to the client. This can be obtained in two ways:

- Adding a default headers — this might be suitable for values that are constant over time and shared across all the
  users — for  an API Key: `myApi.getClient().addDefaultHeader(“API-KEY”, clientConfiguration.getApiKey())`
- Adding headers dynamically — whenever the header values differ from invocation to invocation, the best way to add
  ...
  restTemplate.addInterceptrs(asList(
    (request, response) -> request.getHeaders().add(“Authorization”, …))
  );
...
only](https://openapi-generator.tech/docs/generators/spring/)) implies that the templates must be universal to serve
everyone’s needs.
...
A list of alternative generator tools can be found at [https://openapi.tools/#sdk](https://openapi.tools/#sdk). Pick the best suiting tool based on your needs and FE team preferences.
